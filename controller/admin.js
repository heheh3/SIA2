...
export const appointment_getCompletedCancelled = (req, res) =>{
    ...
    const sqlGet = "SELECT * FROM booking_db as b JOIN users_db as u ON b.patientId = u.user_id WHERE ((b.b_status = 'Cancelled' OR b.b_status = 'R-Completed') AND b.b_paymentStatus = 'Fully-Paid') AND u.user_id = ? AND a_ID BETWEEN ((SELECT a_ID + 1 FROM booking_db as b JOIN users_db as u ON b.patientId = u.user_id WHERE (b.b_status = 'R-Completed' OR b.b_status = 'Completed') AND b.b_paymentStatus = 'Fully-Paid' AND u.user_id = ? AND a_ID < ? ORDER BY a_ID DESC LIMIT 1) OR 0) AND ?"
